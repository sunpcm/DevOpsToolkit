...
3. acme.sh 安装到了错误位置 (`/var/lib/acme/.acme.sh` 而不是 `/var/lib/acme/home/.acme.sh`)
...
- 在所有 acme.sh 相关命令中显式设置 `export HOME=$ACME_HOME/home`
- 确保 acme.sh 安装到正确的位置
...
如果您已经运行过 `acme-init.sh` 但 acme.sh 安装到了错误位置：

```bash
sudo bash acme-fix-install.sh
```
...
- `acme-cleanup.sh` - 清理脚本（已增强）
- `acme-fix-install.sh` - 新增修复脚本
