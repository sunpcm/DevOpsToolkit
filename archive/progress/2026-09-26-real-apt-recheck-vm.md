...
仅在新建的一次性 VM 中复验；inventory 应使用该 VM 的普通账号、测试公钥与
已交叉核对的主机密钥。不得在长期主机移走系统命令。
先安装 zsh 并备份 `/usr/bin/zsh`，再将该命令移至 PATH 外；随后执行：

```bash
ansible-playbook -i <一次性VM inventory> ansible/playbooks/user-only.yml \
  -e '{"user_only_allow_system_dependencies":true,"configure_shell":true,"install_oh_my_zsh":false,"configure_git":false,"configure_uv":false,"configure_node":false,"configure_go":false,"configure_linuxbrew":false}'
```

须使用 ansible-core 2.21.4、项目 `ansible/ansible.cfg` 与锁定 collections；
预期为 apt 任务 `ok`，最终缺失命令断言失败，而不是网络失败或 sudo 失败。
执行前后对 HOME 普通文件运行 `find <HOME> -xdev -type f -exec sha256sum {} + | sort`
并比较结果；完成后恢复命令并删除精确的一次性 VM。

本次原始日志保留在宿主临时目录（不保证永久保存），SHA256 如下：

- Playbook 日志：`22494f609478be15d2f3322ed99742a0c6ad9554e1f0ac8e676f21bca718eff1`。
- HOME 前后列表均为：`c41ff813ed2702d523bf3f8910d75ee7fe64b3906e9dd0a16a36e75e065a61d5`。

关键输出已记录于上文；摘要用于核对原件，不能替代原始日志或新的真实执行。
